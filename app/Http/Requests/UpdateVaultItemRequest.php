...
class UpdateVaultItemRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            'type' => ['sometimes', 'required', 'string', Rule::in(['auth', 'note', 'card', 'api_key', 'ssh_key', 'env_file'])],
            'title' => ['sometimes', 'required', 'string', 'max:255'],
            'folder_id' => ['nullable', 'integer', 'exists:folders,id'],
            'favorite' => ['sometimes', 'boolean'],
            'status' => ['sometimes', 'string', Rule::in(['active', 'archived', 'deleted'])],
        ...
        if ($type === 'auth') {
            ...
            $rules['env_content'] = ['sometimes', 'required', 'string', 'max:50000'];
            $rules['visibility_rules'] = ['sometimes', 'array'];
        }
